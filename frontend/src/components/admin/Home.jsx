...
import { getAllBicycles, deleteBicycle } from "../../api/index";
import "./index.css"; // Import styles used in Home.jsx
import HomePageCarousel from "../common/HomePageCarousel";
import NoContent from "../common/NoContent";
...
function Home() {
  ...
  const handleDelete = async (bicycleId) => {
    try {
      await deleteBicycle(bicycleId);
      setBicycles((prev) =>
        prev.filter((bicycle) => bicycle.bicycle_id !== bicycleId)
      );
    } catch (error) {
      console.error("Error deleting bicycle:", error);
    }
          ...
          {bicycles.map((bicycle, index) => (
            ...
            >
              <Card.Body>
                ...
                <Card.Text>
                  Username: {bicycle.added_by_username}
                </Card.Text>
                <Card.Text>
                  User Type: {bicycle.added_by_usertype}
                </Card.Text>
                <Card.Text>
                  Created Time:{" "}
                  {convertTimestampToDateTime(bicycle.created_time)}
