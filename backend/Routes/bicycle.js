...
// // Get all bicycles (admin only)
// router.get("/all", verifyJwtToken, verifyAdmin, async (req, res) => {
//   try {
//     const bicycles = await Bicycle.find().populate("addedByUserId", "firstName lastName username usertype").sort({ createdTime: -1 });
//     res.status(200).json({ message: "Bicycles fetched successfully", data: bicycles });
//   } catch (error) {
//     res.status(500).json({ message: "Internal server error" });
//   }
// });
...
//     res.status(200).json({ message: "Available bicycles fetched successfully", data: bicycles });
//   } catch (error) {
//     res.status(500).json({ message: "Internal server error" });
//   }
// });
